...
class UploadController extends Zend_Controller_Action {
	...
	public function processAction()
	{
		...
		/* this doesn't really work by now
		 ...
		 if (preg_match('/\\.tmp$/', $file) && (filemtime($filePath) < time() -
		 $maxFileAge))
		 @unlink($filePath);
		 }
		 ...
		 die('{"jsonrpc" : "2.0", "error" : {"code": 100, "message": "Failed to open temp
		 directory."}, "id" : "id"}');
		 */
		...
		// Handle non multipart uploads older WebKit versions didn't support multipart in
		// HTML5
		...
		if (isset($exifArray['GPSLatitudeRef'])) {// Prüfe ob GPS Daten vorhanden sind
			$gpsLatNS = $exifArray['GPSLatitudeRef'];
			// N oder S ?
		} else {
			die('{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "Keine GPS Daten im Bild"}, "id" : "id"}');
		}
		...
		$gpsheight = isset($exifArray['GPSAltitude']) ? (int)$exifArray['GPSAltitude'] : Application_Model_gpsTools::getHeight($gpsLatKoord, $gpsLongKoord);
