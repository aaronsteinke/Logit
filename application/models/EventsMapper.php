...
class Application_Model_EventsMapper {
	...
	public function create($gcal_id, $event_title, $event_start, $event_end, $user_id) {
		...
		$data = array('gcal_id' => $gcal_id, 'event_title' => $event_title, 'event_start' => $event_start, 'event_end' => $event_end, 'user_id' => $user_id);
	...
	private function createObjekt($result) {
		...
		if (is_object($result)) {
			$obUser = new Application_Model_Events($result -> id, $result -> gcal_id, $result -> event_title, $result -> event_start, $result -> event_end, $result -> user_id);
		} else {
			$obUser = new Application_Model_Events($result['id'], $result['gcal_id'], $result['event_title'], $result['event_start'], $result['event_end'], $result['user_id']);
		}
		return $obUser;
	...
	public function connect_Pics($user_id, $event_start, $event_end) {
