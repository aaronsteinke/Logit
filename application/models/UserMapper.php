...
class Application_Model_UserMapper {
	...
	private function createObjekt($result)
	{
		...
		if (is_object($result)) {
			$obUser = new Application_Model_User($result -> id, $result -> username, $result -> lastname, $result -> firstname, $result -> sex, $result -> profilepic, $result -> email, $result -> reg_time, $result -> last_login, $result -> hits, $result -> birthday, $result -> gcal_calendar_id);
		} else {
			$obUser = new Application_Model_User($result['id'], $result['username'], $result['lastname'], $result['firstname'], $result['sex'], $result['profilepic'], $result['email'], $result['reg_time'], $result['last_login'], $result['hits'], $result['birthday'], $result['gcal_calendar_id']);
		}
		return $obUser;
